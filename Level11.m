...
@synthesize dialog_start03 = _dialog_start03;
...
-(id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        _dialog_start03 = [SKLabelNode  labelNodeWithFontNamed:@"EuphemiaUCAS-Bold"];
        _dialog_start03.name = @"dialog_start03";
        _dialog_start03.fontSize = 8;
        _dialog_start03.text = @"给我小考试吧！Quiz~";
        
        _dialog_start03.fontSize = 12;
        _dialog_start03.fontColor = [SKColor colorWithRed:1 green:0.1 blue:0 alpha:1];
        _dialog_start03.position = CGPointMake(CGRectGetMinX(self.frame)+80, CGRectGetMinY(self.frame)+50);
        _dialog_start03.zPosition = 1; // sets it on foreground
        ...
        button_dialog.position = CGPointMake(_dialog_start03.position.x,_dialog_start03.position.y+ padding *_dialog_start03.frame.size.height);
        
        [self addChild: _dialog_start03];
        ...
        _go_back.position = CGPointMake(_dialog_start03.position.x, 0.5*(CGRectGetMinY(self.frame) + _dialog_start03.position.y ));
        ...
        _sentence1d.position = CGPointMake((_sentence1b.position.x)+ 0.5*(_sentence1b.frame.size.width + _sentence1d.frame.size.width) + abs(buffer* _sentence1b.frame.size.width - pad2.frame.size.width), _sentence1a.position.y);
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (UITouch *touch in touches) {
        ...
        for (SKNode *node in nodes) {
            ...
            /*if( [node.name isEqualToString:@"dialog_start03"]){
                ...
                Dialog11 *newScene = [Dialog11 sceneWithSize:spriteView.bounds.size];
            ...
            */
            ...
            if (([node.name isEqualToString:@"shenmeshihou"] ) && (node.hidden== false)){
                
                [self show_chinese_translation:@"什么时候" pinyin:@"shénme shíhòu" english:@"runs" touch_loc:touchLocationInView ];
